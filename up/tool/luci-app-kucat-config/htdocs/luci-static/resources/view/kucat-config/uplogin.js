...
return view.extend({
    ...
    render: function(data) {
        ...
        m = new form.Map('kucat', _('Kucat Login Background Configuration'),
            _('Here you can upload and manage background images for the kucat theme Login page.'));

        // 上传区域 - 只有一个
        s = m.section(form.TypedSection, null, _('Upload Login Background (available space: %1024.2mB)')
            .format(data[1].avail * 1024),
            _('You can upload files such as gif/jpg/mp4/png/webm/webp files, to change the Login background.'));
        s.addremove = false;
        s.anonymous = true;

        o = s.option(form.Button, '_upload_bg', _('Upload Login Background'),
